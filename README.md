...
| Universal geometric nuclear-reaction theory | Closed four-sector foundation spanning free nucleons, deuterium, the mass-three pair, and helium-4, including ordered transport, exact conservation structure, and the complete transport-polarization source space | Universal [theory closure](#universal-geometric-nuclear-reaction-theory) with fixed-law, zero-refit transfer evidence in AlterBBN, PRyMordial, and PArthENoPE. The official PArthENoPE 3.0 campaign completed 695/695 full-network rows and passed the declared Reaction-20 D/H, He-3/H, and Li-7/H shared-endpoint gates across six density anchors. Separate clean-room reconstructions in both PArthENoPE and PRyMordial then recovered the fixed rank-4 source-to-observable architecture from native network physics without inherited numerical response data, froze each construction before held-out execution, and passed all 6/6 held-out native validation rows across the same three preregistered density anchors under dual numerical profiles with no refit. The two clean-room results deepen the derivational interpretation of the already completed transfer evidence while preserving code-local currents, source kernels, normalizations, and solver details as implementation-specific. The separate full-rank AlterBBN susceptibility and native-physics qualification remains pending |
...
|---|---|---|
| [AlterBBN v2](https://doi.org/10.1016/j.cpc.2019.106982) | Native Reaction-20 response surface, endpoint propagation, and code-local current instrumentation | Source-kernel and current quantities remain code-local; the separate full-rank susceptibility and deeper native-physics qualification remains pending |
...
PArthENoPE and PRyMordial therefore now provide two independently implemented native-network reconstructions with the same qualitative result: the fixed rank-4 geometric source architecture can be recovered from native reaction dynamics and its construction-frozen observable operator can transfer prospectively across held-out baryon density. This does **not** assert that the two networks have identical numerical operator matrices, current normalizations, Jacobians, trajectories, or solver internals. Their code-local quantities remain implementation-specific.

### Continuing validation and extension

A separate six-anchor Reaction-21 reserve-channel result supports transfer beyond the commissioning reaction without changing the fixed operator. With the full-network PArthENoPE validation and the clean-room native-physics reconstructions in both PArthENoPE and PRyMordial now complete, the next deeper qualification stage is AlterBBN. That work will apply the same construction-freeze-held-out discipline while preserving AlterBBN's own code-local current normalization, source-kernel structure, and solver behavior. It is an independent third-network derivational test and does not reopen the completed PArthENoPE or PRyMordial authorities.
...
- completion of the universal geometric nuclear-reaction architecture, together with numerical-rigidity testing, native AlterBBN qualification, matched-uniform PRyMordial replication, the completed official PArthENoPE 3.0 full-network 695-row Reaction-20 validation, and subsequent clean-room native-physics reconstructions in both PArthENoPE and PRyMordial, each passing 6/6 held-out native rows across three preregistered density anchors under dual numerical profiles with no refit;
...
- transition to the universal nuclear-reaction research paper while the next independent native-physics reconstruction and full-rank susceptibility qualification proceeds in AlterBBN under the fixed geometry and immutable coefficient package;
...
Current nuclear work is focused on preserving the completed PArthENoPE full-network authority and the completed clean-room authorities in both PArthENoPE and PRyMordial, carrying the same deeper source-to-observable qualification into AlterBBN, and developing the universal research paper from the clean accepted evidence chain. The fixed geometry and immutable coefficient package remain unchanged across construction, independent zero-refit validation, cross-network transfer, structural diagnostics, and quarantined results. Broader CPTG development continues through higher-resolution CMB projections, larger same-aperture cluster samples, and manuscript/report consolidation.
...
At nuclear scale, CPTG now has a closed four-sector reaction architecture and zero-refit transfer evidence across AlterBBN, PRyMordial, and PArthENoPE. The completed official PArthENoPE 3.0 campaign provides a 695-row full-network validation of the fixed Reaction-20 D/H, He-3/H, and Li-7/H shared-endpoint response across six density anchors. Subsequent clean-room reconstructions in both PArthENoPE and PRyMordial independently recovered the rank-4 source-to-observable architecture from native network physics without inherited numerical response data, froze each construction before held-out execution, and passed all 6/6 native validation rows across the same three preregistered density anchors under dual numerical profiles with no refit. PRyMordial additionally completed the reconstruction using only baseline native trajectories, native analytic Jacobians, local source isolation, and direct variational propagation, with zero integrated perturbed response rows. Together these results strengthen the native-physics interpretation of the earlier transfer evidence while preserving code-local currents, source kernels, Jacobians, trajectories, normalizations, and solver details as implementation-specific. The detailed theory, evidence boundaries, validation authorities, and next AlterBBN qualification stage are described in the [Universal Geometric Nuclear-Reaction Theory](#universal-geometric-nuclear-reaction-theory) section; only the unreleased universal reaction formulas remain reserved for the final research paper.
